...
from .proposed_loaders import DatasetMapper_Perturb
...
def _test_loader_from_config(cfg, dataset_name, mapper=None):
    ...
    if mapper is None:
        if cfg.SEV is not None:
            sev= cfg.SEV
        else:
            sev = 3
        if cfg.DATASETS.DATALOADER_MODE:
            mapper = DatasetMapper_Perturb(cfg, False)
            mapper.set_up_mode(mode=cfg.DATASETS.DATALOADER_MODE, sev=sev)
        else:
