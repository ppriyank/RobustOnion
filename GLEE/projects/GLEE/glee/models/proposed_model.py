...
from .glee_model import GLEE_Model, agg_lang_feat

from .transformer_decoder.maskdino_decoder import TRANSFORMER_DECODER_REGISTRY, MaskDINODecoder
from detectron2.config import configurable

from detectron2.projects.glee.data.proposed_utils import normalize, save_image
from einops import rearrange, repeat

@TRANSFORMER_DECODER_REGISTRY.register()
class MaskDINODecoder_Proposed(MaskDINODecoder):
    @configurable
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        
        self.num_classes.update({'flicker': 100})
        self.label_enc.update({'flicker': self.obj365_label_enc})
        
      
class GLEE_Model_Proposed(GLEE_Model):

    def __init__(self, cfg, matcher, device, video_info, contras_mean):
        super().__init__(cfg, matcher, device, video_info, contras_mean)
         
        for p in self.text_encoder.parameters():
            p.requires_grad = False

        if hasattr(self, 'text_encoder_teacher'):
            for p in self.text_encoder_teacher.parameters():
                p.requires_grad = False

        if self.lang_encoder:
        ...
        for p in self.mask_sptial_embed.parameters():
            p.requires_grad = False
        
        if hasattr(self.backbone, 'simfp_3'):
            for p in self.backbone.simfp_3.parameters():
                p.requires_grad = False
        if hasattr(self.backbone, 'simfp_4'):
            for p in self.backbone.simfp_4.parameters():
                p.requires_grad = False
        if hasattr(self.backbone, 'simfp_5'):
            for p in self.backbone.simfp_5.parameters():
                p.requires_grad = False
        
        for p in self.pn_indicator.parameters():
            p.requires_grad = False
            
        self.no_mask_tasks.append("flicker")
        print("===")
    ...
    def forward(self, images, prompts, task, targets=None, batch_name_list=None, is_train = True, visual_prompt_type='scribble'):
        ...
        if self.cfg.MODEL.GRADCAM:
            mask_features, _, multi_scale_features, zero_loss = self.pixel_decoder(features, masks=None, early_fusion = early_semantic)
        else:
            # bz = len(images)//2
...
class GLEE_Model_DUMP(GLEE_Model):
    
    def __init__(self, cfg, matcher, device, video_info, contras_mean):
        super().__init__(cfg=cfg, matcher=matcher, device=device, video_info=video_info, contras_mean=contras_mean)
        self.mode = None 
    ...
    def forward(self, images, prompts, task, targets=None, batch_name_list=None, is_train = True, visual_prompt_type='scribble'):
        ...
        if isinstance(images,torch.Tensor):
            features, intermediate_laters = self.backbone(images)
        else:
            features, intermediate_laters = self.backbone(images.tensor)

        # save_image ( normalize(images.tensor) , "temp.png" ) 
        BACKBONE = intermediate_laters
        ...
        # print([features[e].shape for e in features])
        # torch.Size([1, 256, 192, 192])  
        # torch.Size([1, 256, 96, 96])
        # torch.Size([1, 256, 48, 48])
        # torch.Size([1, 256, 24, 24])

        NECK = [features['p5'], features['p6']]
        #  [g(x[3]), g(x[3]), g(x[3]), H( g(x[3]) ) ]
        NECK = [rearrange(e, "B C H W -> B C (H W)").mean(-1).cpu().numpy() for e in NECK]
        ...
        ####### EARLY FUSION # MaskDINOEncoder
        # type(self.pixel_decoder)
        mask_features, _, multi_scale_features, zero_loss = self.pixel_decoder.forward_features(features, masks=None, early_fusion = early_semantic)

        # print([e.shape for e in multi_scale_features])
        #  [g(x[3]), g(x[3]), g(x[3]), H( g(x[3]) ) ]
        ENCODED = [rearrange(e, "B C H W -> B C (H W)").mean(-1).cpu().numpy()   for e in multi_scale_features]
        # print([e.shape for e in ENCODED])
        # ENCODED = rearrange( multi_scale_features[-1] , "B C H W -> B C (H W)").mean(-1) 
        ENCODED = ENCODED[-2:]
