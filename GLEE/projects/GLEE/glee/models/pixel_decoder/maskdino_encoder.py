...
class MSDeformAttnTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None, early_fusion=None):
        ...
        for _, (layer,vl_layer) in enumerate(zip(self.layers, self.vl_layers)):
            if early_fusion:
                # print("1 ===", output["visual"].mean(1)[0,:10])
                # print("1 ===", output["lang"]['hidden'].mean(1)[0,:10])
                output = vl_layer(output)
                # print("2 ===", output["visual"].mean(1)[0,:10])
                # print("2 ===", output["lang"]['hidden'].mean(1)[0,:10])
                output["visual"] = layer(output["visual"], pos, reference_points, spatial_shapes, level_start_index, padding_mask)
                # print("3 ===", output["visual"].mean(1)[0,:10])
                # print("3 ===", output["lang"]['hidden'].mean(1)[0,:10])
            else:
...
class MaskDINOEncoder(nn.Module):
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        if (cfg.MODEL.BACKBONE.NAME  in ['D2_EVA02', 'D2_EVA01' , 'D2_ViT'])  or ('D2_EVA02_' in cfg.MODEL.BACKBONE.NAME):
            ret["ViTBackbone"] = True 
        else:
            ret["ViTBackbone"] = False 
    ...
    def forward_features(self, features, masks, early_fusion=None):
        ...
        # type(self.transformer) glee.models.pixel_decoder.maskdino_encoder.MSDeformAttnTransformerEncoderOnly'>
        ...
        # [e.shape for e in out]
        # [torch.Size([1, 256, 192, 192]), torch.Size([1, 256, 96, 96]), torch.Size([1, 256, 48, 48]), torch.Size([1, 256, 24, 24]), torch.Size([1, 256, 192, 192])]
        for o in out:
            if num_cur_levels < self.total_num_feature_levels:
                multi_scale_features.append(o)
                num_cur_levels += 1
        
        # [e.shape for e in multi_scale_features]
        # [torch.Size([1, 256, 192, 192]), torch.Size([1, 256, 96, 96]), torch.Size([1, 256, 48, 48]), torch.Size([1, 256, 24, 24])]
...
@SEM_SEG_HEADS_REGISTRY.register()
class MaskDINOEncoder_GRADCAM(MaskDINOEncoder):
    
    @autocast(enabled=False)
    def forward(self, features, masks, early_fusion=None):
        """
        :param features: multi-scale features from the backbone
        :param masks: image mask
        :return: enhanced multi-scale features and mask feature (1/4 resolution) for the decoder to produce binary mask
