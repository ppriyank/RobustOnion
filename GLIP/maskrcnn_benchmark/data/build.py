...
from .collate_batch import BatchCollator, BBoxAugCollator, Replica_BatchCollator, Replica_BatchCollator_RelicaEnd, Replica_BatchCollator_Seed
...
def build_dataset(cfg, dataset_list, transforms, dataset_catalog, is_train=True, class_concat=False, extra_args={}):
    ...
    for dataset_id, dataset_name in enumerate(dataset_list, 1):
        ...
        # during training
        cfg.defrost()
        if 'img_folder' in data['args']:
            cfg.DATASETS.PROFILE.append(data['args']['img_folder'])
        cfg.freeze()
        ...
        if cfg.DATASETS.DATALOADER or cfg.DATASETS.TRAIN_DATALOADER or cfg.DATASETS.TEST_DATALOADER \
            or dataset_name == 'kitti_Grounding_Pairs-grounding':
            if cfg.DATASETS.DATALOADER:
                factory = getattr(D, cfg.DATASETS.DATALOADER)
            elif cfg.DATASETS.TRAIN_DATALOADER and is_train:
                factory = getattr(D, cfg.DATASETS.TRAIN_DATALOADER)
            elif cfg.DATASETS.TEST_DATALOADER and (not is_train):
                factory = getattr(D, cfg.DATASETS.TEST_DATALOADER)
            if cfg.DATASETS.DATALOADER_MODE or cfg.DATASETS.TEST_NOISE:
                if not is_train and cfg.DATASETS.TEST_NOISE:
                    args["mode"] = cfg.DATASETS.TEST_NOISE
                else:
                    args["mode"] = cfg.DATASETS.DATALOADER_MODE
            if cfg.DATASETS.N_LR:
                args["N_LR"] = cfg.DATASETS.N_LR
                args["N_BR"] = cfg.DATASETS.N_BR
            if cfg.DATASETS.ONE_NOISE:
                args["ONE_NOISE"] = cfg.DATASETS.ONE_NOISE

        if cfg.DATASETS.DATALOADER_MODE:
            args["mode"] = cfg.DATASETS.DATALOADER_MODE
                
        if cfg.DATASETS.FOG_LEVEL:
            args["fog_level"] = cfg.DATASETS.FOG_LEVEL
...
def make_data_loader(cfg, is_train=True, is_distributed=False, num_replicas=None, rank=None, start_iter=0, only_return_dataset=False, seed_based=False):
    ...
    for di, dataset in enumerate(datasets):
        ...
        if seed_based:
            collator = Replica_BatchCollator_Seed(cfg.DATALOADER.SIZE_DIVISIBILITY)
        elif (cfg.DATASETS.TRAIN_DATALOADER or type(dataset).__name__ == 'VirtualKitti_Grounding_Pairs') and is_train and (not cfg.DATASETS.REPLICA_END):
            collator = Replica_BatchCollator(cfg.DATALOADER.SIZE_DIVISIBILITY)
        elif cfg.DATASETS.REPLICA_END and is_train:
            collator = Replica_BatchCollator_RelicaEnd(cfg.DATALOADER.SIZE_DIVISIBILITY)
        else:
    ...
    # ids = defaultdict(int)
    # for e in data_loader:[ids.__setitem__(a, ids.get(a, 0) + 1) for a in e[2]]
