...
class FPN(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # B1, B2, B3, B4 
        # [(1, 96, H, W), (1, 192, H,W ), (1, 384, H,W), (1, 768, H,W)]
        x_text = None 
        if type(x) is tuple:
        ...
        if self.return_swint_feature_before_fusion:
            ...
            swint_feature_c4 = x[0]

        if self.spp_block:
            last_inner = getattr(self, self.inner_blocks[-1])(self.spp_block(x[-1]))
        else:
            last_inner = getattr(self, self.inner_blocks[-1])(x[-1])
        # last_inner = C1(B4) ## last_inner
        results = []
        results.append(getattr(self, self.layer_blocks[-1])(last_inner))
        # N3 = H1 ( C1(B4) ) ## last_inner

        # x[:-1][::-1] --> B1, B2, B3 --> B3, B2, B1 
        # [e.shape for e in x[:-1][::-1]]
        for feature, inner_block, layer_block in zip(
            ...
            # C2 ( B3 ) # inner_lateral 
            # C1 ( B4 ) ## last_inner == inner_top_down

            # C3 ( B2 ) # inner_lateral
            #  C2 ( B3 ) + C1 ( B4 ) ## last_inner == inner_top_down
        ...
        if isinstance(self.top_blocks, LastLevelP6P7):
            ...
            # B4 , C3(B4)
            # C3(B4) --> x --> p6 :: C4 ( C3(B4) ) --> p7 :: C5 ( C4 ( C3(B4) ) )
            results.extend(last_results)
            # N4 = C4(B4 , C3(B4)) || N5 = C5 ( C4 ( C3(B4) ) ) 
