#!/bin/bash

#SBATCH --job-name=OR_7
#SBATCH --output=ucf_output/slurm-%j.out
#SBATCH --gres-flags=enforce-binding
#SBATCH -p gpu
#SBATCH --exclude=c4-4,c5-2
#SBATCH -C gmem48 --gres=gpu:2 --mem-per-cpu=6G 
#SBATCH --cpus-per-gpu=8

####SBATCH -c8
############## SBATCH -c8
################SBATCH -p gpu --qos=preempt
##################SBATCH -p gpu --qos=day
#################SBATCH -p gpu --qos=short 

# srun --pty --gres=gpu:1 --cpus-per-gpu=8 -C gmem48 --qos preempt bash 
# srun --pty --gres=gpu:1 --cpus-per-gpu=8 -C gmem48 bash 
# srun --pty --gres=gpu:2 --cpus-per-gpu=8 -C gmem48 --exclude=c4-4,c5-2 bash 
# srun --pty --gres=gpu:1 --cpus-per-gpu=8 --qos preempt bash 
# srun --pty --cpus-per-task=8 bash 

nvidia-smi
source ~/.bashrc
CONDA_BASE=$(conda info --base) ; 
source $CONDA_BASE/etc/profile.d/conda.sh
echo -e '\n\n' + "*"{,,,,,,,,,,,,,,,,}
echo $SLURM_JOB_ID $SLURM_JOB_NODELIST
echo $CONDA_DEFAULT_ENV
echo -e '\n\n' + "*"{,,,,,,,,,,,,,,,,}
scontrol write batch_script $SLURM_JOB_ID
mv slurm-$SLURM_JOB_ID.sh ucf_output/
rsync -a ucf_output/slurm-$SLURM_JOB_ID.sh ucf2:~/robustness_object_detection/GLIP/ucf_output/

module load cuda/11.3
conda activate OD
cd ~/robustness_object_detection/GLIP/


PORT=$((RANDOM % 55 + 12345))
while ss -tuln | grep -q ":$PORT"; do
  PORT=$((RANDOM % 55 + 12345))
done
echo "Free port found: $PORT"

export DETECTRON2_DATASETS=/home/c3-0/datasets/flickr_dataset_30k/
export DATASET=/home/c3-0/datasets/flickr_dataset_30k/
export ADD_DATASET=/home/c3-0/datasets/
export TOKENIZERS_PARALLELISM=true 

NUM_GPU=2


if [[ "$SLURM_JOB_NODELIST" == "c4-4" ]]; then
        sbatch ucf_output/slurm-$SLURM_JOB_ID.sh
        exit 1
fi





###################### GLIP-T [5] ########################################################################################
CONFIG=configs/pretrain/glip_Swin_T_O365_GoldG.yaml
WT='MODEL/glip_tiny_model_o365_goldg_cc_sbu.pth'
OUTPUT=ucf_output/GLIP-T_5
MODEL_TYPE=GLIP-T_5

TRAIN_FN=multi_scale_train
DATALOADER=FlickrDataset_MULTI_SCALE
TEST_DATALOADER=COCODataset_Perturb
TECHNIQUE=GeneralizedVLRCNN_PROPOSE
SUBSET=subset3


# ##############################################################################################################################
# ############################################# BASELINE TRAINING (Synthetic)

# # #################### LR-TKO
# BACKBONE_TYPE=LR_TKO
# BATCHSIZE=3

###################### LR-TKO + SR 
# BACKBONE_TYPE=LR_TKO_SR
# BATCHSIZE=3

# ###################### Adapters 
# BACKBONE_TYPE=ADAPTER
# BATCHSIZE=3

# ###################### VPT
# BACKBONE_TYPE=VPT_DEEP
# BATCHSIZE=4

# ###################### LoRA
# BACKBONE_TYPE=LORA
# BATCHSIZE=4


NAME=$MODEL_TYPE-$BACKBONE_TYPE
OUTPUT=ucf_output/$NAME
ROOT_DUMP_FOLDER=EXPS/$NAME




###################### Baselines + Training in pairs of HQ / LQ 
LOSS_MODE="low-res"



# ###################### Augmented Noise 
# NORMAL_NOISES=("pixel_dropout" "iso_blur" "salt_pepper" "jpg_compression" "low-res" "focus_blur" "fog" "chromatic" "motion_blur2" "focus_blur")
# for LOSS_MODE in "${NORMAL_NOISES[@]}"
# do
#   DUMP_FOLDER=$ROOT_DUMP_FOLDER+$LOSS_MODE/
#   rm -rf $DUMP_FOLDER

#   OUTPUT_NOISE=$OUTPUT-$LOSS_MODE
#   printf "\n\n #### $LOSS_MODE \n\n" >> $OUTPUT_NOISE.txt
#   ####### Vanilla Training 
  # CUDA_VISIBLE_DEVICES=0,1 PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python python -W ignore -m torch.distributed.launch --nnodes 1 --nproc_per_node=$NUM_GPU --master_port=$PORT tools/train_net_proposed.py --config-file $CONFIG \
  #     --override_output_dir $DUMP_FOLDER --train_fn=$TRAIN_FN --subset $SUBSET  \
  #     SOLVER.IMS_PER_BATCH $((BATCHSIZE*NUM_GPU)) TEST.IMS_PER_BATCH $NUM_GPU TEST.DURING_TRAINING True MODEL.WEIGHT $WT TEST.EVAL_TASK detection MODEL.DYHEAD.SCORE_AGG "MEAN" MODEL.DYHEAD.FUSE_CONFIG.MLM_LOSS False \
  #     DATASETS.TRAIN_DATALOADER $DATALOADER DATASETS.TEST_DATALOADER $TEST_DATALOADER DATASETS.DATALOADER_MODE $LOSS_MODE \
  #     MODEL.NO_NORM True MODEL.META_ARCHITECTURE $TECHNIQUE MODEL.BACKBONE_TYPE $BACKBONE_TYPE >> $OUTPUT_NOISE.txt
# done

# ###################### Real World Weather (GAN Based)
# BATCHSIZE=3
# NORMAL_NOISES=("snow_model2" "atmospheric" "rain_model2")
# for LOSS_MODE in "${NORMAL_NOISES[@]}"
# do
#   DUMP_FOLDER=$ROOT_DUMP_FOLDER+$LOSS_MODE/
#   rm -rf $DUMP_FOLDER

#   OUTPUT_NOISE=$OUTPUT-$LOSS_MODE
#   printf "\n\n #### $LOSS_MODE \n\n" >> $OUTPUT_NOISE.txt
#   CUDA_VISIBLE_DEVICES=0,1 PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python python -W ignore -m torch.distributed.launch --nnodes 1 --nproc_per_node=$NUM_GPU --master_port=$PORT tools/train_net_proposed.py --config-file $CONFIG \
#     --override_output_dir $DUMP_FOLDER --train_fn=$TRAIN_FN --subset $SUBSET --spawn-method \
#     SOLVER.IMS_PER_BATCH $((BATCHSIZE*NUM_GPU)) TEST.IMS_PER_BATCH $NUM_GPU TEST.DURING_TRAINING True MODEL.WEIGHT $WT TEST.EVAL_TASK detection MODEL.DYHEAD.SCORE_AGG "MEAN" MODEL.DYHEAD.FUSE_CONFIG.MLM_LOSS False \
#     DATASETS.TRAIN_DATALOADER $DATALOADER DATASETS.TEST_DATALOADER $TEST_DATALOADER DATASETS.DATALOADER_MODE $LOSS_MODE \
#     MODEL.NO_NORM True MODEL.META_ARCHITECTURE $TECHNIQUE MODEL.BACKBONE_TYPE $BACKBONE_TYPE DATALOADER.NUM_WORKERS 1 >> $OUTPUT_NOISE.txt
# done


    






# ##############################################################################################################################
# ############################################# Real World

# # ###### WEDGE
# DUMP_FOLDER=$ROOT_DUMP_FOLDER+WEDGE/
# rm -rf $DUMP_FOLDER
# OUTPUT_NOISE=$OUTPUT-WEDGE
# printf "\n\n #### WEDGE \n\n" >> $OUTPUT_NOISE.txt
# Dataset='("wedge_test",)'
# Dataset='("wedge_train-grounding",)'
# BATCHSIZE=10

# ###### Virtual Kitty
# Dataset_TRAIN='("kitti_train-grounding-debug",)'
# Dataset_TRAIN='("kitti_train-grounding",)'
# Dataset_TEST='("kitti_val",)'
# BATCHSIZE=15
# SUFFIX=VK

###### Virtual Kitty Pairs
# Dataset_TRAIN='("kitti_Grounding_Pairs-grounding",)'
# Dataset_TEST='("kitti_val",)'
# BATCHSIZE=15
# SUFFIX=VK-P

# ###### BDDK
# Dataset_TRAIN='("bdd100k_train-grounding",)'
# Dataset_TEST='("bdd100k_val2",)'
# BATCHSIZE=1
# SUFFIX=BDD

# ###### BDDK
Dataset_TRAIN='("bdd100k_all_train-grounding",)'
Dataset_TEST='("bdd100k_val2",)'
BATCHSIZE=2
SUFFIX=BDD-ALL


echo "External dataset ..... "



##################### LR-TKO
# BACKBONE_TYPE=LR_TKO

# ###################### LoRA
# BACKBONE_TYPE=LORA

# ###################### Adapters 
# BACKBONE_TYPE=ADAPTER

# ##################### LR-TKO
# BACKBONE_TYPE=LR_TKO_SR

# ###################### VPT
# BACKBONE_TYPE=VPT_DEEP



################################################ Modified backbone + layer norms
# NAME=$MODEL_TYPE-$BACKBONE_TYPE-$SUFFIX
# OUTPUT_NOISE=ucf_output/$NAME
# DUMP_FOLDER=EXPS/$NAME
# rm -rf $DUMP_FOLDER
# printf "\n\n #### $SUFFIX \n\n" >> $OUTPUT_NOISE.txt
# CUDA_VISIBLE_DEVICES=0,1 PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python python -W ignore -m torch.distributed.launch --nnodes 1 --nproc_per_node=$NUM_GPU --master_port=$PORT tools/train_net_proposed.py --config-file $CONFIG --no-display \
#   --override_output_dir $DUMP_FOLDER --train_fn=$TRAIN_FN \
#   SOLVER.IMS_PER_BATCH $((BATCHSIZE*NUM_GPU)) TEST.IMS_PER_BATCH $NUM_GPU TEST.DURING_TRAINING True MODEL.WEIGHT $WT TEST.EVAL_TASK detection MODEL.DYHEAD.SCORE_AGG "MEAN" MODEL.DYHEAD.FUSE_CONFIG.MLM_LOSS False \
#   DATASETS.TRAIN $Dataset_TRAIN DATASETS.TEST $Dataset_TEST  \
#   MODEL.NO_NORM True MODEL.META_ARCHITECTURE $TECHNIQUE MODEL.BACKBONE_TYPE $BACKBONE_TYPE DATALOADER.NUM_WORKERS 2 SOLVER.USE_AMP True >> $OUTPUT_NOISE.txt


################################################ Finetune
# ## WORKS :: Starting the evaluations at 4700
# NAME=$MODEL_TYPE-$BACKBONE_TYPE-$SUFFIX-L2
# OUTPUT_NOISE=ucf_output/$NAME
# DUMP_FOLDER=EXPS/$NAME
# printf "\n\n #### $SUFFIX \n\n" >> $OUTPUT_NOISE.txt
# CUDA_VISIBLE_DEVICES=0,1 PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python python -W ignore -m torch.distributed.launch --nnodes 1 --nproc_per_node=$NUM_GPU --master_port=$PORT tools/train_net_proposed.py --config-file $CONFIG --no-display \
#   --override_output_dir $DUMP_FOLDER --train_fn=$TRAIN_FN \
#   SOLVER.IMS_PER_BATCH $((BATCHSIZE*NUM_GPU)) TEST.IMS_PER_BATCH $NUM_GPU TEST.DURING_TRAINING True MODEL.WEIGHT $WT TEST.EVAL_TASK detection MODEL.DYHEAD.SCORE_AGG "MEAN" MODEL.DYHEAD.FUSE_CONFIG.MLM_LOSS False \
#   DATASETS.TRAIN $Dataset_TRAIN DATASETS.TEST $Dataset_TEST  \
#   MODEL.NO_NORM True MODEL.META_ARCHITECTURE $TECHNIQUE MODEL.BACKBONE_TYPE $BACKBONE_TYPE DATALOADER.NUM_WORKERS 2 SOLVER.USE_AMP True \
#   MODEL.DYHEAD.NUM_CLASSES 10 MODEL.DYHEAD.FUSE_CONFIG.USE_CLASSIFICATION_LOSS True >> $OUTPUT_NOISE.txt


################################################ ## Per Noise 
# BATCHSIZE=10
# # NOISES=("15-deg-left" "15-deg-right" "30-deg-left" "30-deg-right" "clone" "fog" "morning" "overcast" "rain" "sunset" )
# # NOISES=("15-deg-left" "15-deg-right")
# NOISES=("15-deg-right")
# # NOISES=("30-deg-left" "30-deg-right")
# # NOISES=("15-deg-right")
# # NOISES=("30-deg-right")
# # NOISES=("clone" "fog")
# NOISES=("morning" "overcast")
# # NOISES=("rain" "sunset")
# for LOSS_MODE in "${NOISES[@]}"
# do
#   NAME=$MODEL_TYPE-$BACKBONE_TYPE+$LOSS_MODE+$SUFFIX
#   OUTPUT=ucf_output/$NAME
#   DUMP_FOLDER=EXPS/$NAME
#   rm -rf $DUMP_FOLDER
#   printf "\n\n #### $SUFFIX :: $LOSS_MODE \n\n" >> $OUTPUT.txt

#   CUDA_VISIBLE_DEVICES=0,1 PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python python -W ignore -m torch.distributed.launch --nnodes 1 --nproc_per_node=$NUM_GPU --master_port=$PORT tools/train_net_proposed.py --config-file $CONFIG --no-display \
#     --override_output_dir $DUMP_FOLDER --train_fn=$TRAIN_FN \
#     SOLVER.IMS_PER_BATCH $((BATCHSIZE*NUM_GPU)) TEST.IMS_PER_BATCH $NUM_GPU TEST.DURING_TRAINING True MODEL.WEIGHT $WT TEST.EVAL_TASK detection MODEL.DYHEAD.SCORE_AGG "MEAN" MODEL.DYHEAD.FUSE_CONFIG.MLM_LOSS False \
#     DATASETS.TRAIN $Dataset_TRAIN DATASETS.TEST $Dataset_TEST DATASETS.DATALOADER_MODE $LOSS_MODE \
#     MODEL.NO_NORM True MODEL.META_ARCHITECTURE $TECHNIQUE MODEL.BACKBONE_TYPE $BACKBONE_TYPE DATALOADER.NUM_WORKERS 2 SOLVER.USE_AMP True >> $OUTPUT.txt
# done
























# ##############################################################################################################################
# ############################################# Reinforcement Learning Based All noises 

HEAD=VLDyHeadModule_LANG_FUSE
N_LR=3
N_BR=1

EXP_NAME=Pos_Lang_Tri-$N_LR-$N_BR

SUFFIX=RL
TECHNIQUE=GeneralizedVLRCNN_PROPOSE
DATALOADER='FlickrDataset_MULTI_SCALE_SEED'
TRAIN_FN=do_train_rl
BATCHSIZE=3

###### Train on Flicker
SUBSET=subset3

###### BDDK (Validation, 100 sampler per class)
Dataset_TEST='("bdd100k_val2",)'

###### Strategy Model (100 Samples)
ext_dataset="bdd100k_train-grounding" 
SUFFIX=BDD-$SUFFIX

# #################### LR-TKO
BACKBONE_TYPE=LR_TKO



NAME=$MODEL_TYPE-$BACKBONE_TYPE-$$EXP_NAME-$SUFFIX
OUTPUT_NOISE=ucf_output/$NAME
DUMP_FOLDER=EXPS/$NAME
# rm -rf $DUMP_FOLDER
# printf "\n\n #### $SUFFIX \n\n" >> $OUTPUT_NOISE.txt


NUM_GPU=2
CUDA_VISIBLE_DEVICES=0,1 PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python python -W ignore -m torch.distributed.launch --nnodes 1 --nproc_per_node=$NUM_GPU --master_port=$PORT tools/train_net_proposed_rl.py --config-file $CONFIG  \
    --override_output_dir $DUMP_FOLDER --subset $SUBSET --ext_dataset $ext_dataset --spawn-method --no_noise_pred $N_LR --train_fn=$TRAIN_FN \
    --policy_model_lr 0.1 DATASETS.TRAIN_DATALOADER $DATALOADER  \
    SOLVER.IMS_PER_BATCH $((BATCHSIZE*NUM_GPU)) TEST.IMS_PER_BATCH $NUM_GPU TEST.DURING_TRAINING True MODEL.WEIGHT $WT TEST.EVAL_TASK detection MODEL.DYHEAD.SCORE_AGG "MEAN" MODEL.DYHEAD.FUSE_CONFIG.MLM_LOSS False \
    DATASETS.TEST $Dataset_TEST MODEL.NO_NORM True MODEL.META_ARCHITECTURE $TECHNIQUE MODEL.BACKBONE_TYPE $BACKBONE_TYPE DATALOADER.NUM_WORKERS 1 SOLVER.USE_AMP True \
    DATASETS.N_LR $(($N_LR-1)) DATASETS.N_BR $N_BR
    
    #  >> trial.txt



# TECHNIQUE=GeneralizedVLRCNN_RL
# MODEL.RPN_ARCHITECTURE $HEAD 

# METHOD=LR_TKO-Neg_Lang_Tri-2-FL
# MODEL.RPN.NO_FUSE one_anchor
# EXP_NAME=Neg_Lang_Tri-$N_LR-$N_BR
    
# LR_TKO-Lang-Tri-2-FL
# LR_TKO-DiFu-Tri-FL

# Baseline Triplet loss 
# Train on BDDK 
# Triplet loss instead of CLIP (github)
     
  
# --no-display
# step_factor=1
# adv_weight=1
# r1=0
# r3=1
# r2=1
# clip_grad_norm=1
# sim_weight=0.5


# clean optimize action policy logic 
# smaller validation set to 100 samples 
# train of strategy is train of target model
# let it explore more
# explore more aggresive 
# Strategy also sees target dataset 



















rsync -a ucf_output/* ucf2:~/robustness_object_detection/GLIP/ucf_output/
# rsync -a ucf0:~/robustness_object_detection/GLIP/ucf_output/* ~/robustness_object_detection/GLIP/ucf_output/



# cd ~/robustness_object_detection/GLIP/
# sbatch Script/train_proposed.sh


