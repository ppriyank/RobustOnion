...
from train_net_proposed import compute_flops
...
def main():
    ...
    parser.add_argument("--re-validate", default=False, action='store_true',)
    ...
    if args.task_config:
        ...
        for task_config in all_task_configs:
            ...
            if log_dir:
                ...
                for output_folder, dataset_name, data_loader_val in zip(output_folders, dataset_names, data_loaders_val):
                    if args.re_validate:
                        from maskrcnn_benchmark.engine.inference import inference_re_validate
                        inference_re_validate(
                            model=model,
        ...
        if log_dir:
            ...
            for output_folder, dataset_name, data_loader_val in zip(output_folders, dataset_names, data_loaders_val):
                if args.DUMP:
                    from maskrcnn_benchmark.engine.inference import inference_dump
                    inference_dump( model=model, data_loader=data_loader_val, dataset_name=dataset_name, iou_types=iou_types,
                        box_only=cfg.MODEL.RPN_ONLY and (cfg.MODEL.RPN_ARCHITECTURE == "RPN" or cfg.DATASETS.CLASS_AGNOSTIC), 
                        device=cfg.MODEL.DEVICE, expected_results=cfg.TEST.EXPECTED_RESULTS, expected_results_sigma_tol=cfg.TEST.EXPECTED_RESULTS_SIGMA_TOL,
                        output_folder=output_folder, cfg=cfg)
                elif args.re_validate:
                    from maskrcnn_benchmark.engine.inference import inference_re_validate
                    inference_re_validate(
                        model=model,
                        data_loader=data_loader_val,
